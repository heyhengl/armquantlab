...
struct DemoRenderer {
    static func main() async throws {
        ...
        let silentURL = outputDirectory.appendingPathComponent("armquantlab-demo-silent.mp4")
        let srtURL = outputDirectory.appendingPathComponent("ArmQuantLab_Demo_en.srt")
        try FileManager.default.createDirectory(
            at: outputDirectory,
            withIntermediateDirectories: true
        )
        for url in [silentURL, outputURL, timelineURL, srtURL]
        ...
        for (index, scene) in manifest.scenes.enumerated() {
            ...
            let sceneDuration = durations[index] + CMTime(
                seconds: gapSeconds,
                preferredTimescale: 600
            )
        ...
        for (index, scene) in manifest.scenes.enumerated() {
            ...
            cursor = cursor + durations[index] + CMTime(
                seconds: gapSeconds,
                preferredTimescale: 600
            )
        }
